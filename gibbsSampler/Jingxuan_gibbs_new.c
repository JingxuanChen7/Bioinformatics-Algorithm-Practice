...
#define PLATEAU 15 // if score does not change for PLATEAU times, then finish
                // modified 11/20/2019
...
{
    int i, j, upd, tmplMotif, lMotif, maxMotif[cSeq], cplateau = 0;
    char **pssmMotif;
    seedOut samplerOut;
    rawSeq *tmpSeq;
    ...
    //    tmpscore = samplerScore(inputSeq, cSeq, lSeq, lMotif);
        ...
        if (tmpscore > score)
        {
            cplateau = 0; // initialize cplateau
            score = tmpscore;
            for (i = 0; i < cSeq; i++)
