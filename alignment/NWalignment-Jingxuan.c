...
int main(int argc, char *argv[])
{
    ...
    while(!feof(fp1))
    {
        if (isalpha(seq1[lseq1] = fgetc(fp1)) == 0) // if not letter, do not fget to seq
            continue;
        lseq1++;
        
    }
    seq1[lseq1] = '\0';
    seq1 = realloc(seq1, (lseq1 + 1) * sizeof(char));
    for(i = 0; i < lseq1; i++)
    {
    ...
    while (!feof(fp2))
    {
        if (isalpha(seq2[lseq2] = fgetc(fp2)) == 0)
            continue;
        lseq2++;
    }
    seq2[lseq2] = '\0';
...
long **matrixScore (int **mMatch, long lseq1, long lseq2, int sGap)
{
    ...
    {
        mScore[lseq2-i][lseq1] = i * sGap;
    }
    for (j = 1; j <= lseq1 + 1; j++)
    {
        mScore[lseq2][lseq1-j] = j * sGap;
    }
    for (i = 1; i < lseq2 + 1; i++)
    {
        for (j = 1; j < lseq1 + 1; j++)
        {
            right = mScore[lseq2-i][lseq1-j+1] + sGap;
            down = mScore[lseq2-i+1][lseq1-j] + sGap;
            diag = mScore[lseq2-i+1][lseq1-j+1] + mMatch[lseq2-i][lseq1-j];
            mScore[lseq2 - i][lseq1 - j] = findMax(right, down, diag);
