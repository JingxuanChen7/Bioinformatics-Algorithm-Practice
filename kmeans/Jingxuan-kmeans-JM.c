...
int main(int argc, char *argv[])
{
    ...
    } else
    {
        ...
        sscanf(argv[2], "%d", &kmax);             //JM: you can also use kmax=atoi(argv[2]). atoi converts a string to int, atol to long, atof to double.
        /* skip the first line */
        fscanf(fp, "%*[^\n]\n", NULL);
        npoint = 0;
        while (!feof(fp))
...
kmeansOut kmeans(int k, int ndimension, int npoint, point pt[], int seed, char* pinputFile, int checkMin)
{
    ...
    kmOut.seed = seed;
    srand48(kmOut.seed);      //JM: I prefer using time to set the seed or have the initial seed as a command-line parameter.
                              //JM: Hardcoding the seed would produce the saem output when you run the program again with teh same input, which is generally not desirable
    ...
    for (i = 0; i < k; i++)
    {
        ...
        {
            if (i <= j)
//                fprintf(fp, "\t%.4lf\t", dist(ct[i], ct[j], k));            //JM: this was wrong
                fprintf(fp, "\t%.4lf\t", sqrt(dist(ct[i], ct[j], ndimension)));    //JM: this is right
            else
                fprintf(fp, "\t\t");
        }
...
int cmpCt(int npoint, int ndimension, point pt[])    //JM: this works but it is easier to check if the WCSS got smaller and stop if it did not
{
    int i, j;
    for (i = 0; i < npoint; i++)
    {
...
double** kmeans_plus(int k, int ndimension, int npoint, point pt[])
{
    ...
    for (i = 0; i < k; i++)
    {
        ...
        else // initialize next few centroids
        {
            ...
            for (l = 0; l < npoint; l++)
            {
                pt2ct = dist(pt[l].data, ct[i - 1], ndimension);   //JM: if you do not need pt2ct later there is nothing wrong with sumDist+=dist(pt[l].data,ct[i-1],ndimension);
                sumDist += pt2ct;
            }

            for (l = 0; l < npoint; l++)
            {
                pt2ct = dist(pt[l].data, ct[i - 1], ndimension);  //JM: This works but I'd rather store teh distances calculated 
                                                                  //JM: in the previous loop so that I don't have to recalculate them here.
                len += (pt2ct / sumDist);
                if (len > randNum)
                {
                    x = l;
                    for (j = 0; j < ndimension; j++)
                    {
                        ct[i][j] = pt[x].data[j];     //JM: why not ct[i][j] = pt[l].data[j] and skip the x=l earlier?
                        //printf("the cent is %lf\n", ct[i][j]);
                    }
                    break;
                }
