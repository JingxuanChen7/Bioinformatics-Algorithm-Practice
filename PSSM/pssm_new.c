/*
    ...
    pthread and math included in this code,
    include -lm and -lpthread while compiling
*/
...
void *findMotif(void *arg); // screening through genome
...
int main(int argc, char *argv[])
{
    ...
    seqInt = (int *)malloc(lseq * sizeof(int));
    ...
    while (!feof(fp2))
    {
        ...
        if (c == 'A' || c == 'a')
            ...
            seqInt[len] = 3;
        else if (isalpha(c) == 0) // if not letter, do not fget to seq
            continue;
        else 
            seqInt[len] = -1;
        len++;
    ...
    /* find possible motif in genome using pthread mode */
    args *positive = (args*)malloc(sizeof(args));
    positive->seqInt = seqInt;
    // positive->seqChar = seq;
    positive->len = len;
    positive->lMotif = lMotif;
    positive->minScore = minScore;
    positive->scoreMatrix = scoreMatrix;
    positive->strand = '+';

    args *negative = (args *)malloc(sizeof(args));
    negative->seqInt = seqInt;
    // negative->seqChar = seq;
    negative->len = len;
    negative->lMotif = lMotif;
    negative->minScore = minScore;
    negative->scoreMatrix = cscoreMatrix;
    negative->strand = '-';

    pthread_t thread_pos, thread_neg;
    pthread_create(&thread_pos, NULL, findMotif, (void *)positive);
    // findMotif(seq, scoreMatrix, lMotif, len, minScore, '+');
    // pthread_create(&thread_neg, NULL, findMotif, (void *)negative);
    findMotif((void *)negative);
    pthread_join(thread_pos, NULL);
    // pthread_join(thread_neg, NULL);
...
void *findMotif(void* arg)
{
    ...
    maxpos = aarg->len - aarg->lMotif; // max first position
    pseq = aarg->seqInt;
    // pseqChar = aarg->seqChar;
    // point to first element
    motifSeq = (int *)malloc((aarg->lMotif) * sizeof(int));
    // seq = (char *)malloc((aarg->lMotif + 1) * sizeof(char));

    // motifSeq[aarg->lMotif] = '\0';
    for ( i = 0; i < maxpos; i++)
    {
        // strncpy(motifSeq, pseq + i, (aarg->lMotif * sizeof(char)));
        motifSeq = pseq + i;
        score = seqScore(aarg->scoreMatrix, aarg->lMotif, motifSeq);
        if (score > aarg->minScore)
        {
            seq = int2seq(motifSeq, aarg->lMotif);
            // seq = pseqChar + i;
            // seq[aarg->lMotif] = '\0';
            printf("%ld\t%ld\t%c\t%s\t%.3lf\n", i + 1, i + aarg->lMotif + 1, aarg->strand, seq, score);
        }
    }
    return NULL;
}
